...
if (isset($_SESSION['nivel'])){
	...
	$_SESSION["operativa-vista"] = "devolucion"; // variable para saber en que vista esta y asi no destruir sus variables de trabajo
...
<script type="text/javascript">
	...
	function buscarAjaxBien(){
		...
		ajax.open("POST","../../control/movimientos/c_devolucion.php",true);
		ajax.onreadystatechange=function(){
			if(ajax.readyState==4){
				document.getElementById("datosAjax1").innerHTML = ajax.responseText;
			}
		}
		ajax.setRequestHeader("Content-type","application/x-www-form-urlencoded");

		/* preparo mis variables para la condicion del bien */
		var departamento_ajax = document.getElementById('dep').value;
		var fec_asig_ajax = document.getElementById('f_Dev').value;

		/**********************/
		ajax.send("operacion=busquedaAjax&dep_a="+departamento_ajax+"&fec_devol="+fec_asig_ajax); //paso variable con estatus Nulo y nombre
	}
...
<form action="../../control/movimientos/c_devolucion.php" method="POST" name="envio_form" id="envio_form" autocomplete="off">
<!-- ********************* Validar Período Abierto ****************** -->
<?php if(isset($_SESSION["Nom_periodo_mostrar"])){ ?>
	<span class="periododevolucion-tour" id="mostrar-periodo">Período: <?php echo $_SESSION["Nom_periodo_mostrar"]; ?></span>
<?php }else{ ?>
	<span class="periododevolucion-tour">
...
<!-- ***************************************** Datos de Entrada del bien nacional *******************************************-->
	<tr id="datos-entrada-bien1">
		...
		<input type="hidden" name="idPeriodAnulacion" value="<?php if(isset($_SESSION["TrazabPeriodoAnteriorDev"])) if($_SESSION["TrazabPeriodoAnteriorDev"] == '1') echo '1'; ?>">
		<!-- variable para el numero de recepcion -->
		<input type="hidden" name="n_Dev" value="<?php if(isset($_SESSION['n_Dev'])) echo $_SESSION['n_Dev'];   ?>">
		<!-- variable para el saber que tipo de mivimiento estoy realizando -->
		<input type="hidden" name="tipo_de_mov" value="3"> <!-- devolucion -->
		<!-- variable utilizada para controlar la anulacion -->
		<input type="hidden" name="control_anulacion" value="<?php if(isset($_SESSION['anulacionDev'])) if($_SESSION['anulacionDev'] == '1') echo '1'; if(isset($_SESSION['anulacionDev'])) if($_SESSION['anulacionDev'] == '2') echo '2'; ?>">
		<input type="hidden" name="control_anulacion_Trazabilidad" value="<?php if(isset($_SESSION['anulacionTrazabilidadDev'])) if($_SESSION['anulacionTrazabilidadDev'] == '1') echo '1'; ?>">
		
		<td id="r-paddig" style='width:35%;' class="numerodevolucion-tour"> 
			<label>Nro. Devolución:<span class="asterisco">*</span> </label>
			<input type="text" class="CampoMov" onBlur="cambio_mayus(this)" id="n_Devolucion" name="n_Devolucion" readonly="readonly" placeholder="Ingrese el n° de la devolución" value="<?php if(isset($_SESSION['n_Devolucion'])) echo $_SESSION['n_Devolucion'] ;?>">	
		</td>
		<td id="r-paddig" style="width:32%;" class="responsabledevolucion-tour">
			<label>Responsable Devolución:<span class="asterisco">*</span>  </label>
			<?php
		...
		<td id="r-paddig" class="fechadevolucion-tour">
			<label>Fecha de Devolución:<span class="asterisco">*</span>  </label> 
			<input type="text" class="CampoMov" onBlur="cambio_mayus(this);" id="f_Dev" name="f_Dev" readonly="readonly" placeholder="Día-Mes-Año" value="<?php if(isset($_SESSION['f_Dev'])) echo $_SESSION['f_Dev'] ;?>">
		</td>
		
	</tr>
	<tr id="datos-entrada-bien2">
		<td id="r-paddig" style="width:32%;" class="departamento-tour">
			<label>Departamento: <span class="asterisco">*</span></label><br>
			<?php
				include_once('../../control/configuracion/c_dep.php');
				$ArrayDep = array();
			    $combDep = "";
				if(isset($_SESSION['cod_dep_dev'])){
					$combDep = $_SESSION['cod_dep_dev'];
				}
		...
		<td id="r-paddig" style="width:32%;" class="responsabledepartamento-tour">
			...
			<?php
				...
				$cod_per = "";
				if(isset($_SESSION['id_personal_dep_dev']))
					$cod_per = $_SESSION['id_personal_dep_dev'];
				else
					$cod_per = "";
						  
				$cboPersonalDep = DibCombPersonalDep($cod_per);
						
				foreach ($cboPersonalDep as $elementos) 
					echo $elementos;		   
			?>
		</td>
		<td id="r-paddig" class="motivo-tour">
			<label>Motivo:<span class="asterisco">*</span>  </label><br>
			<?php
				include_once('../../control/configuracion/combos/c_combos_motivo.php');
				$cboMotivoAsi = array();
				$cod_mot = "";
				if(isset($_SESSION['id_motivo_dev'])){
					$cod_mot = $_SESSION['id_motivo_dev'];
				}else{
					$cod_mot = "";
				}
						  
				$cboMotivoAsi = DibCombMotivoDevolucion($cod_mot);
		...
		<tr id="datos-entrada-bien3" class="observaciondevolucion-tour">
	 	<td id="r-paddig" colspan="2">
			<label>Observación: </label>
			<textarea id="textareaCabecera" onBlur="cambio_mayus(this)" class="CampoMov" readonly="readonly" name="obser_cabecera" placeholder="Ingrese Observación, este campo es opcional " ><?php if(isset($_SESSION['obser_cabecera_Dev'])) echo $_SESSION['obser_cabecera_Dev'] ;?></textarea>
		</td>
	</tr> 
	</tr>
...
<!-- ****************************************** Datos para el Rigistro del bien nacional *************************************************-->
	<tr>
		<td colspan="3">
			<div class="caption"><?php if(!isset($_SESSION["GlobalDetalleDev"])){?><span id="iconos-recep-bien-mas" title="clic para mostrar la vista de bien" onclick="MostrarBien()" class="icon-plus"></span><span id="iconos-recep-bien-men" title="clic para no mostrar la vista del bien" onclick="CerrarBien()" class="icon-minus"></span> <?php } ?>Devolución: Datos del Bien Nacional <span title="Clic para ver ayuda guiada"class="icon-magic-wand element-devolucion2 ayuda-local-frm ayudaguiada2-tour"></span> </div>
		</td>
	</tr>
	<tr id="datos-entrada-bien4" class="biennacionalesdisponibles-tour">
		<td id="r-paddig" style="width:47%;">
			<label class="clickdedevolucion-tour" >Bien Nacional: <span class="asterisco">*</span> <button type="button" disabled="disabled" name="busBienAsig" id="busBienAsig" title="clic para consultar Bienes Nacionales Disponibles" class="buscarBien" name="buscarCodBien" onclick="validarBtnConsBienDev()" >Clic para consultar los Bienes Nacionales a Devolver &nbsp; <span class="icon-search"></span></button> </label>
...
<!-- ****************************************** Cierre del los Datos para el Rigistro del bien nacional *************************************************-->
<tr><td colspan="3">
	<div class="caption asignaciondetalle-tour">Devolución: Detalle del Bien Nacional</div>
	<div id="cabecera-detalle" class="todoslosdetalles-tour"> 
		...
		<span id="n9"><?php if(isset($_SESSION["GlobalDetalleDev"])) echo "Observación"; else echo "Ubicación"; ?></span>
		<span id="n10"><?php if(isset($_SESSION["GlobalDetalleDev"])) echo "Condición"; else echo "Observación"; ?></span>
		<span id="n"></span>
	</div>
</td></tr>
<!-- ******************************************* Detalle del bien nacional *************************************************-->
			...
			<div id="topLink">
				<table id="detalle" style="text-align:center;">
					<!-- pintar detalle dinamico -->
						<?php if(isset($_SESSION["GlobalDetalleDev"])) echo $_SESSION["GlobalDetalleDev"]; ?>
				</table>
			</div> 
		</td>
	</tr>
		...
		function quitarBien(nodoPadre,value){ 
			...
			function agregar(){
				...
				for(var i = 1; i <= canti; i++){
					...
					var checkboxModal = document.envio_Modal1[variablecheckbox];
				...
				var canti = document.envio_Modal1.cantidadFilas.value;
				for(var i = 1; i <= canti; i++){
					/****** obtengo codigo desde la modal *****/
					/** checkbox seleccionado **/
					var textcheckbox = "chec"; var valorcheckbox = i;
					...
					if( checkboxModal.checked ){
						...
						var codigo = document.envio_Modal1[variableCod];
...
<!-- ******************************************* Botonera *************************************************-->
	<tr id="botonera" class="botonera-tour">
		<td  align="center" colspan="3"><br/>
			...
			<!-- Incluyo la botonera -->
			<button type="button" class="botoneranuevo-tour" name="inc" value="Incluir" onclick="General_Devolucion(this.value)"><span id="iconos" class="icon-file-empty"></span><span>Nuevo</span></button>				
			<button type="button" class="botoneraconsultar-tour" value="Consultar"  name="bus"  onclick="Listar2()" ><span id="iconos" class="icon-search"></span>Consultar</button>
			<button type="button" class="botoneraguardar-tour" value="Grabar" name="grab" onclick="General_Devolucion(this.value)" disabled="disabled"><span id="iconos" class="icon-clipboard"></span><span>Guardar</span></button>					
			<button type="button" class="botoneraactivar-tour" value="Anular" name="anu" onclick="General_Devolucion(this.value)" disabled="disabled"><span id="iconos" class="icon-folder-minus"></span><span>Anular</span></button>
			...
			<button type="button" class="botoneracancelar-tour" value="Cancelar" name="cancel" onclick="General_Devolucion(this.value)" disabled="disabled"><span id="iconos" class="icon-cross"></span><span>Cancelar</span></button>
...
<!-- ******************************************* Script para la Botonera *************************************************-->
			<script>	
				...
				if(frm.control_anulacion.value == 2 && frm.control_anulacion_Trazabilidad.value != 1){
					...
					frm.n_Devolucion.style.cursor = "not-allowed";
					frm.id_personal_recep.style.cursor = "not-allowed";
					frm.f_Dev.style.cursor = "not-allowed";
					frm.dep.style.cursor = "not-allowed";
					frm.id_personal_asig.style.cursor = "not-allowed";
					frm.id_motivo_dev.style.cursor = "not-allowed";
...
<!-- ModalTipoAnulacionDevol -->
<div class="ModalTipoAnulacionDev">
			...
			<tr>
				<td> 
					<?php
						...
						$cboMotivoAnu = DibCombMotivoAnulacionDevol($cod_mot);
...
<!-- incluyo el mensajde de que los campos son obligatorios -->
	...
	<?php  
		...
		include_once("../consultaModal/movimientos/v_devolucion_consultarDeparBien.php");

		include_once("../consultaModal/movimientos/v_devolucion_consultar.php");  
	?>
<?php

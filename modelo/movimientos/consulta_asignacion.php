...
class clsAsignacionConsul Extends clsConexion{
	...
	function Consultar_asignacion(){
		$sql = "SELECT mov.id_mov,mov.nro_document,mov.fecha_reg,mov.hora_reg,mov.fecha_mov,mov.id_tipo_mov,mov.id_per,mov.id_usuario,mov.id_motivo_mov,mov.id_periodo,mov.id_dep,mov.id_resp_dep,mov.observacion_mov, per.ced_per as ced_per_asig,per.nom_per as nom_per_asig,per.ape_per as ape_per_asig,user.login,mot_mov.des_motivo_mov,
		...
		WHERE (mov.id_tipo_mov=tipo_mov.id_tipo_mov) and (mov.id_tipo_mov='2') and (mov.id_per=per.id_per) and (mov.id_resp_dep=resp.id_per) and (mov.id_usuario=user.id_usuario) and (mov.id_motivo_mov=mot_mov.id_motivo_mov) and (mov.id_periodo=peri.id_periodo)

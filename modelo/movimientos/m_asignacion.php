...
class ClsAsignacion extends clsConexion{
	private $G;
	...
	function incluir(){
		...
		$sql = "SELECT max(id_mov) as max FROM movimiento WHERE status='1' and id_usuario='".$_SESSION['id_usuario']."' and nro_document='".$this->G["n_Asignacion"]."' and fecha_reg='".$fecha."'";
		...
		$dato = $this->converArray($tupla); //ultimo id de la tabla movimiento
		 ...
		 if( $this->como_va() ){
				...
				/* inserto en la tabla bien nacional */
		 		...
		 		$sql = "UPDATE bien_nacional SET id_cond='2' WHERE cod_bien='".$this->G["cod_bien_R_array"][$c]."'";
		 		$this->ejecuta($sql);
	...
	function Consultar_Cant_Movimientos(){
		$sql = "SELECT count(id_mov) as cantidad FROM movimiento WHERE id_tipo_mov=2 and status=1";
		$rs = $this->ejecuta($sql);
		if( $this->como_va() ){
	...
	function ConsulCabRece($idAsig){
		$sql = "SELECT mv.observacion_mov FROM movimiento as mv WHERE id_mov='".$idAsig."'";
	...
	function anularAsignacion($idAsig,$MotAnulacion){
		$transaccion = false; // inicializo la variable en false
		$this->inicio_trans(); // inicializo la trasaccion
		//actualizo el status				
		...
		$sql = "UPDATE movimiento SET status='0' WHERE id_mov='".$idAsig."'";
		$this->ejecuta($sql);

		if( $this->como_va() ){
			/* inserto los datos correspondiente de la anulacion en la tabla movimientos */
			$sql = "UPDATE movimiento SET id_usuario_anulacion='".$_SESSION["id_usuario"]."', fecha_anulacion='".$fecha."', id_motivo_anulacion='".$MotAnulacion."' WHERE id_mov='".$idAsig."'";
			$this->ejecuta($sql);

			if( $this->como_va() ){

				/* actulizo el status en la tabla detalle del movimiento */
				$sql = "UPDATE detalle_movimiento SET status='0' WHERE id_mov='".$idAsig."'";
				$this->ejecuta($sql);
				...
				if( $this->como_va() ){
					...
					$sql = "SELECT id_bien FROM detalle_movimiento WHERE id_mov='".$idAsig."'";
					$rs = $this->ejecuta($sql);
					if( $this->como_va() ){

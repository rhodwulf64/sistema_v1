...
class clsFechaHora extends clsConexion{
	...
	public function traerFecha($fec){
	...
	public function traerFecha2($fec){
		$fechaServer = substr($fec,8,2).'-'.substr($fec,5,2).'-'.substr($fec,0,4);
		return $fechaServer;
	}
	function SubirFechaServer($fecha){
		$fechaSubida = substr($fecha,6,4).'-'.substr($fecha,3,2).'-'.substr($fecha,0,2);
		return $fechaSubida;
	}
	...
	public function ConverArray($rs){

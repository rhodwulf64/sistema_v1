...
class ClsDevolucionDepMotivo extends conex{
	private $datos;
	function ClsDevolucionDepMotivo(){
		$this->conex();
		$this->datos="";
	}

	function buscar($cod,$resp,$POST){
		$this->datos=$POST;
		...
		//todo los tipo de bienes
			$sql= "SELECT bn.cod_bien,tb.des_tbien,mm.des_motivo_mov,bn.des_bien,bn.serial_bien,bn.modelo,dpto.nom_dep,CONCAT('C.I ',p.ced_per,' ',p.nom_per,' ',p.ape_per) as conc,mar.nom_marca,m.fecha_mov FROM detalle_movimiento as dm INNER JOIN bien_nacional as bn INNER JOIN movimiento as m  INNER JOIN tipo_bien as tb  INNER JOIN motivo_movimiento as mm  INNER JOIN marca as mar  INNER JOIN departamento as dpto  INNER JOIN personal as p INNER JOIN tipo_movimiento as tm WHERE

dm.id_bien=bn.id_bien and dm.id_mov=m.id_mov and bn.id_marca=mar.id_marca 
and m.id_motivo_mov=mm.id_motivo_mov and p.id_dep=dpto.id_dep and m.id_resp_dep=p.id_per
 and bn.id_tbien=tb.id_tbien and
  m.id_resp_dep='".$resp."' and tm.id_tipo_mov=m.id_tipo_mov and dpto.id_dep='".$cod."'
   and mm.id_motivo_mov='".$this->datos['motivo_mov']."' and tm.id_tipo_mov=3";
//echo $sql;
			$rs=$this->ejecuta($sql);
		if(isset($this->datos['motivo_mov']) && $this->datos['motivo_mov']==""){
		 $sql= "SELECT bn.cod_bien,tb.des_tbien,mm.des_motivo_mov,bn.des_bien,bn.serial_bien,bn.modelo,dpto.nom_dep,CONCAT('C.I ',p.ced_per,' ',p.nom_per,' ',p.ape_per) as conc,mar.nom_marca,m.fecha_mov FROM detalle_movimiento as dm INNER JOIN bien_nacional as bn INNER JOIN movimiento as m  INNER JOIN tipo_bien as tb  INNER JOIN motivo_movimiento as mm  INNER JOIN marca as mar  INNER JOIN departamento as dpto  INNER JOIN personal as p INNER JOIN tipo_movimiento as tm WHERE
...
//echo $sql;
		...
		$html='
			...
			*{
			...
			<h4 align="center">Listado de Bienes Nacionales Devueltos por Motivo</h4><br>
            </div>
			</div>
			<!--inicio de tabla del Listado -->
			<p id="negrita">Responsable del Dpto:'.$tupla1['conc'].'</p><p style="margin-top:-16px;" align="right">Fecha de Devolución: '.$fecha_mov.'</p>
			<p>Motivo: '.$tupla1['des_motivo_mov'].'</p>
			<br>

...
class ClsAsignacionDepMotivo extends conex{
	private $datos;
	function ClsAsignacionDepMotivo(){
		$this->conex();
		$this->datos="";
	}

	function buscar($cod,$resp,$cod_tmov){
		...
		//todo los tipo de bienes
			$sql= "SELECT bn.cod_bien,tb.des_tbien,bn.des_bien,bn.serial_bien,bn.modelo,dpto.nom_dep,CONCAT('C.I ',p.ced_per,' ',p.nom_per,' ',p.ape_per) as conc,mar.nom_marca,m.fecha_mov,mm.des_motivo_mov


FROM detalle_movimiento as dm INNER JOIN bien_nacional as bn ON dm.id_bien=bn.id_bien INNER JOIN movimiento as m ON dm.id_mov=m.id_mov INNER JOIN motivo_movimiento as mm INNER JOIN tipo_bien as tb ON bn.id_tbien=tb.id_tbien INNER JOIN tipo_movimiento as tm ON m.id_tipo_mov=tm.id_tipo_mov INNER JOIN marca as mar ON bn.id_marca=mar.id_marca INNER JOIN departamento as dpto ON m.id_dep=dpto.id_dep INNER JOIN personal as p ON m.id_resp_dep=p.id_per
	WHERE dpto.id_dep='".$cod."' AND p.id_per='".$resp."' and m.id_motivo_mov=mm.id_motivo_mov and m.id_motivo_mov='".$cod_tmov."' and tm.id_tipo_mov=2";
//echo $sql;
			$rs=$this->ejecuta($sql);
		if($cod_tmov==""){
		 $sql= "SELECT bn.cod_bien,tb.des_tbien,bn.des_bien,bn.serial_bien,bn.modelo,dpto.nom_dep,CONCAT('C.I ',p.ced_per,' ',p.nom_per,' ',p.ape_per) as conc,mar.nom_marca,m.fecha_mov,mm.des_motivo_mov
...
FROM detalle_movimiento as dm INNER JOIN bien_nacional as bn ON dm.id_bien=bn.id_bien INNER JOIN movimiento as m ON dm.id_mov=m.id_mov INNER JOIN motivo_movimiento as mm INNER JOIN tipo_bien as tb ON bn.id_tbien=tb.id_tbien INNER JOIN tipo_movimiento as tm ON m.id_tipo_mov=tm.id_tipo_mov INNER JOIN marca as mar ON bn.id_marca=mar.id_marca INNER JOIN departamento as dpto ON m.id_dep=dpto.id_dep INNER JOIN personal as p ON m.id_resp_dep=p.id_per
	WHERE dpto.id_dep='".$cod."' AND p.id_per='".$resp."' and m.id_motivo_mov=mm.id_motivo_mov  and tm.id_tipo_mov=2";
//echo $sql;
			$rs=$this->ejecuta($sql);
		}
		...
		$html='
			...
			*{
			...
			<h4 align="center">Listado de Bienes Nacionales Asignados al Departamento por Motivo</h4><br>
            </div>
			</div>
			<!--inicio de tabla del Listado -->
			<p id="negrita">Responsable del Dpto:'.$tupla1['conc'].'</p><p style="margin-top:-16px;" align="right">Fecha de Asignación: '.$fecha_mov.'</p>

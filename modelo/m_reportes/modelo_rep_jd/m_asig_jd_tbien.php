...
class ClsAsignacionDepTbien extends conex{
	private $datos;
	function ClsAsignacionDepTbien(){
		$this->conex();
		$this->datos="";
	}
...
FROM detalle_movimiento as dm INNER JOIN bien_nacional as bn ON dm.id_bien=bn.id_bien INNER JOIN movimiento as m ON dm.id_mov=m.id_mov INNER JOIN tipo_bien as tb ON bn.id_tbien=tb.id_tbien INNER JOIN tipo_movimiento as tm ON m.id_tipo_mov=tm.id_tipo_mov INNER JOIN marca as mar ON bn.id_marca=mar.id_marca INNER JOIN departamento as dpto ON m.id_dep=dpto.id_dep INNER JOIN personal as p ON m.id_resp_dep=p.id_per 
	WHERE dpto.id_dep='".$cod."' AND p.id_per='".$resp."' and tm.id_tipo_mov=2
 ";
 //echo $sql;
			$rs=$this->ejecuta($sql);
		}
		...
		$html='
			...
			*{
			...
			<h4 align="center">Listado de Bienes Nacionales Asignados al Departamento</h4><br>
            </div>
			</div>
			<!--inicio de tabla del Listado -->
			<p id="negrita">Responsable del Dpto:'.$tupla1['conc'].'</p><p style="margin-top:-14px;" align="right">Fecha de Asignación: '.$fecha_mov.'</p>
			<br>

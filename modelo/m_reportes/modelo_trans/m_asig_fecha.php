...
class ClsAsignacion extends conex{
	...
	function buscar_fecha($fecha1,$fecha2){
		...
		$content='
			...
			*{
			...
			<h4 align="center">Asignaciones por Fechas</h4>
            </div>
			</div>
        ...
        $f1=$this->envio_fecha($fecha1);
        $f2=$this->envio_fecha($fecha2);
        $sql=" SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,mov.id_dep,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us WHERE mov.fecha_mov BETWEEN '$f1' and '$f2' and (m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and tmov.id_tipo_mov=mov.id_tipo_mov and  tmov.cod_tipo_mov='21' and tmov.id_tipo_mov=2 and bn.status=1  and mov.id_per=per.id_per and mov.status=1 and mov.id_usuario=us.id_usuario)";
        //echo $sql;
	       $rs=$this->ejecuta($sql); //se pinta el reporte la parte del bien nacional row es como tupla luego le cmabio el nombre
             while($row=$this->TraerArreglo($rs)) {
                ...
                // if(isset($total)){  //pinto el footer pie de pagina
                	$medio='
                    <table border="1" align="center" class="td-individual"  style="border-collapse:collapse;">
                		...
                		';
           	 			$pdf->WriteHTML($medio);
        		// }else{
        		// 	echo "<script>alert('NO HAY ASIGNACIONES POR FECHA');</script>";
        		// 	return false;
        		// }
		$footer="<page_footer>
		<style>
    ...
    //**********************************FIN DE LA FUNCION PARA BUSCAR ASIGNACIONES POR FECHA**********************//

    //*********************FUNCTION PARA BUSCAR ASIGNACIONES POR DEPARTAMENTO**********************************//
	...
	function consulta(){
		 ...
		 INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us WHERE dep.nom_dep='$nom' and (m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and prov.id_prov=mov.id_prov and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario)";

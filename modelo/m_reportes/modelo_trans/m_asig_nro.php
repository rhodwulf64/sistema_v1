...
class ClsAsignacion extends conex{
	private $datos;
	function ClsAsignacion(){
		$this->conex();
		$this->datos="";
	}
	function buscar($POST, $nro_document,$nroAsigAutoinc,$respAsigReport){
		$this->datos=$POST;
		date_default_timezone_set('America/Caracas');
		include_once('../../../reportes/html2pdf/html2pdf.class.php');
		$pdf= new HTML2PDF('L','A4','fr'); // instancio la la clase HTML2PDF

		if(isset($this->datos['nro']) && $this->datos['nro']==""){ //consulta del listado General
			...
			$sql="SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov,dep.id_dep,dep.nom_dep, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN tipo_movimiento as tmov INNER JOIN departamento as dep INNER JOIN personal as per INNER JOIN usuario as us WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_dep=dep.id_dep and mov.id_usuario=us.id_usuario and tmov.cod_tipo_mov=21 and tmov.id_tipo_mov=2 and bn.status=1 and mov.status='1'  order by bn.cod_bien asc";
	       	$rs=$this->ejecuta($sql);
		}else if(isset($this->datos['nro']) && $this->datos['nro']!=""){ // consulta cuando se ingresa en la caja el nro de asignacion
			$sql_header="SELECT s.cod_sed, s.nom_sed, org.cod_org, org.nom_org, org.cod_ud, org.nom_ud,sis.direccion,p.nom_per,p.ape_per,p.ced_per,car.nom_car FROM sede as s INNER JOIN organizacion as org INNER JOIN sistema as sis INNER JOIN movimiento as m INNER JOIN personal as p INNER JOIN cargo as car WHERE s.id_sed=sis.id_sed and s.id_org=org.id_org and m.id_per=p.id_per and m.id_per in (SELECT id_per FROM movimiento as mmm WHERE mmm.nro_document='".$this->datos['nro']."') and car.id_car=p.id_cargo";
			$rs_header=$this->ejecuta($sql_header);

			$sql="SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov,dep.id_dep,dep.nom_dep, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN tipo_movimiento as tmov INNER JOIN departamento as dep INNER JOIN personal as per INNER JOIN usuario as us WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_dep=dep.id_dep and mov.id_usuario=us.id_usuario and tmov.cod_tipo_mov=21 and tmov.id_tipo_mov=2 and bn.status=1 and mov.status='1' and mov.nro_document='".$this->datos['nro']."'  order by bn.cod_bien asc";
	       	$rs=$this->ejecuta($sql);
		}else if(isset($nro_document) && $nro_document!=""){
			$sql_header="SELECT s.cod_sed, s.nom_sed, org.cod_org, org.nom_org, org.cod_ud, org.nom_ud,sis.direccion,p.nom_per,p.ape_per,p.ced_per,car.nom_car FROM sede as s INNER JOIN organizacion as org INNER JOIN sistema as sis INNER JOIN movimiento as m INNER JOIN personal as p INNER JOIN cargo as car WHERE s.id_sed=sis.id_sed and s.id_org=org.id_org and m.id_per=p.id_per and m.id_per=".$respAsigReport." and car.id_car=p.id_cargo";
			$rs_header=$this->ejecuta($sql_header);

			$sql="SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov,dep.id_dep,dep.nom_dep, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN tipo_movimiento as tmov INNER JOIN departamento as dep INNER JOIN personal as per INNER JOIN usuario as us WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_dep=dep.id_dep and mov.id_usuario=us.id_usuario and tmov.cod_tipo_mov=21 and tmov.id_tipo_mov=2 and bn.status=1 and mov.status='1' and mov.nro_document='".$nro_document."' and mov.id_mov=".$nroAsigAutoinc." order by bn.cod_bien asc";
	       	$rs=$this->ejecuta($sql);
		}else{
			return false;
		}
		...
		<img src="../../../public/img/logoMinisterio.jpg"/>
			...
			<h4 align="center">Asignación de Bienes Nacionales</h4>
            </div>
			</div>

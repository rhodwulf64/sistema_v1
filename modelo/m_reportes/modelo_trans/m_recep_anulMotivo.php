...
class ClsRecepcionAnul extends conex{
	...
	function buscar($POST){
      ...
      $sql="SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,prov.des_prov,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN proveedor as prov INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us INNER JOIN motivo_movimiento as mm WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov   and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and prov.id_prov=mov.id_prov and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario and mov.id_motivo_anulacion='".@$this->datos['id_motivo_anu']."' AND tmov.id_tipo_mov=1 AND tmov.cod_tipo_mov=21 AND mm.id_motivo_mov=mov.id_motivo_mov AND mov.status=0 AND bn.status=0 order by bn.cod_bien asc";

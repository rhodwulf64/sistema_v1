...
class  ClsDesincorporacion extends conex{
	...
	if(isset($this->datos['fecha1']) && $this->datos['fecha1']=="" && isset($this->datos['fecha2']) && $this->datos['fecha2']==""){
		$sql_header="SELECT s.cod_sed, s.nom_sed, org.cod_org, org.nom_org, org.cod_ud,peri.nom_periodo, org.nom_ud,sis.direccion,p.nom_per,p.ape_per,p.ced_per,car.nom_car 
			FROM sede as s INNER JOIN organizacion as org INNER JOIN sistema as sis INNER JOIN movimiento as m INNER JOIN personal as p INNER JOIN cargo as car INNER JOIN periodo as peri
			 WHERE s.id_sed=sis.id_sed and s.id_org=org.id_org and m.id_per=p.id_per and car.id_car=p.id_cargo and m.id_periodo=peri.id_periodo";
			$rs_header=$this->ejecuta($sql_header);

			$f1=$this->envio_fecha($this->datos['fecha1']);
       		$f2=$this->envio_fecha($this->datos['fecha2']);
        	$sql="SELECT bn.cod_bien,mov.observacion_mov,bn.des_bien,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p  INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us WHERE mov.fecha_mov BETWEEN '$f1' and '$f2' and (m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo  and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario)and tmov.cod_tipo_mov='51' and mov.status=1 order by bn.cod_bien asc";
       		$rs=$this->ejecuta($sql);
		}else if(isset($this->datos['fecha1']) && $this->datos['fecha1']!="" && isset($this->datos['fecha2']) && $this->datos['fecha2']!=""){
			$sql_header="SELECT s.cod_sed, s.nom_sed, org.cod_org, org.nom_org, org.cod_ud,peri.nom_periodo, org.nom_ud,sis.direccion,p.nom_per,p.ape_per,p.ced_per,car.nom_car FROM sede as s INNER JOIN organizacion as org INNER JOIN sistema as sis INNER JOIN movimiento as m INNER JOIN personal as p INNER JOIN cargo as car INNER JOIN periodo as peri WHERE s.id_sed=sis.id_sed and s.id_org=org.id_org and m.id_per=p.id_per and car.id_car=p.id_cargo and m.id_periodo=peri.id_periodo";		
			$rs_header=$this->ejecuta($sql_header);
		...
		$content='
			...
			*{
			...
			<h4 align="center">Desincorporación de Bienes Nacionales por Fecha</h4>
                    ...
                    $valor=$row['precio'];
                 	...
                 	<table border="1" align="center" style="border-collapse:collapse;margin-left:-5px;margin-top:-1px;"  >
				        <tr align="center">
					       ...
					       <td width="92" rowspan="2">'.$row['observacion_mov'].'</td>
				        </tr>
        ...
        // 	echo "<script>alert('NO HAY DESINCORPORACIONES POR ESTA FECHA');</script>";
        // 	return false;
        // }
		$footer="<page_footer>
		<style>

...
class ClsRecepcion extends conex{
	...
	function buscar_tmov($tmov){
		...
		$sql_header="SELECT o.cod_org,o.nom_org,o.cod_ud,o.nom_ud,s.cod_sed,s.nom_sed,p.des_prov,per.nom_per,per.ape_per,per.ced_per,c.nom_car,sis.direccion,mm.des_motivo_mov FROM organizacion AS o INNER JOIN sede AS s INNER JOIN proveedor AS p INNER JOIN personal AS per INNER JOIN cargo AS c INNER JOIN sistema AS sis INNER JOIN movimiento AS mov INNER JOIN motivo_movimiento AS mm WHERE mm.id_motivo_mov='$tmov' AND (o.id_org=s.id_org AND mov.id_prov=p.id_prov) AND (mov.id_per=per.id_per AND per.id_cargo=c.id_car) AND (mov.id_motivo_mov=mm.id_motivo_mov) AND sis.id_sed=s.id_sed";
		...
		$content='
			...
			*{
			...
			<h4 align="center">Recepción</h4>
                 ...
                 <p id="negrita">Proveedor: '.$tupla2['des_prov'].'</p>
        ...
        $sql="SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,prov.des_prov,tmov.cod_tipo_mov,tmov.nom_tipo_mov,mm.des_motivo_mov,mm.tipo_motivo,mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN proveedor as prov INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us INNER JOIN motivo_movimiento as mm WHERE mm.id_motivo_mov='$tmov' and (m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and prov.id_prov=mov.id_prov and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario and tmov.cod_tipo_mov=21 and tmov.id_tipo_mov=1 and mov.id_motivo_mov=mm.id_motivo_mov)";
        //echo $sql;
	       $rs=$this->ejecuta($sql); //se pinta el reporte la parte del bien nacional row es como tupla luego le cmabio el nombre
             while($row=$this->TraerArreglo($rs)) {
                    ...
                    $valor=$row['precio'];
                 	...
                 	<table border="1" align="center" style="border-collapse:collapse;margin-left:-5px;margin-top:-1px;"  >
				        <tr align="center">
					       ...
					       <td width="355" rowspan="1">'.$row['des_bien'].' '.$marca.' '.$modelo.'</td>
                    ...
                    @$total=$total+$valor; //pinto el footer pie de pagina
                $medio='
		...
		*{
		...
		$pdf->Output('f.pdf','I'); //salida al navegador del pdf

	}
	function consulta(){
		 $sql="SELECT bn.cod_bien,bn.des_bien,m.nom_marca,bn.modelo,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,prov.id_prov,prov.des_prov,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn
		 INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c
		 INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p INNER JOIN proveedor as prov
		 INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us
		 WHERE prov.id_prov=mov.id_prov and (m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo and prov.id_prov=mov.id_prov and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario)";

...
class ClsDesincorporacion extends conex{
	private $datos;

	function ClsDesincorporacion(){
		$this->conex();
		$this->datos="";
	}
	function buscar($POST,$des_document,$idAutoDes,$idRespDes){
		$this->datos=$POST;

		include_once('../../../reportes/html2pdf/html2pdf.class.php');
		$pdf= new HTML2PDF('L','A4','fr'); // instancio la la clase HTML2PDF

		if(isset($this->datos['nro']) && $this->datos['nro']==""){
			...
			$sql="SELECT bn.cod_bien,bn.des_bien,mov.observacion_mov,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p  INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo  and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario and tmov.cod_tipo_mov='51' and mov.status='1' order by bn.cod_bien asc";
	       	$rs=$this->ejecuta($sql);
		}else if(isset($this->datos['nro']) && $this->datos['nro']!=""){
			...
			$sql="SELECT bn.cod_bien,bn.des_bien,mov.observacion_mov,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p  INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo  and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario and mov.nro_document='".$this->datos['nro']."' and tmov.cod_tipo_mov='51' and mov.status='1' order by bn.cod_bien asc";
	       	$rs=$this->ejecuta($sql);
		}else if(isset($des_document) && $des_document!=""){
				$sql_header="SELECT s.cod_sed, s.nom_sed, org.cod_org, org.nom_org, org.cod_ud, org.nom_ud,sis.direccion,p.nom_per,p.ape_per,p.ced_per,car.nom_car FROM sede as s INNER JOIN organizacion as org INNER JOIN sistema as sis INNER JOIN movimiento as m INNER JOIN personal as p INNER JOIN cargo as car WHERE s.id_sed=sis.id_sed and s.id_org=org.id_org and m.id_per=p.id_per and m.id_per=".$idRespDes." and car.id_car=p.id_cargo";
			$rs_header=$this->ejecuta($sql_header);

			$sql="SELECT bn.cod_bien,bn.des_bien,mov.observacion_mov,m.nom_marca,bn.modelo,bn.serial_bien,tb.cod_tbien,tb.des_tbien,bn.precio,bn.observacion_bien,bn.fecha_ent, c.nom_cond,p.nom_periodo,tmov.cod_tipo_mov,tmov.nom_tipo_mov, mov.nro_document,per.nom_per,us.login FROM bien_nacional as bn INNER JOIN marca as m INNER JOIN tipo_bien as tb INNER JOIN condicion as c INNER JOIN detalle_movimiento as dm INNER JOIN movimiento as mov INNER JOIN periodo as p  INNER JOIN tipo_movimiento as tmov INNER JOIN personal as per INNER JOIN usuario as us WHERE m.id_marca=bn.id_marca and tb.id_tbien=bn.id_tbien and c.id_cond=bn.id_cond and dm.id_mov=mov.id_mov and dm.id_bien=bn.id_bien and mov.id_periodo=p.id_periodo  and tmov.id_tipo_mov=mov.id_tipo_mov and mov.id_per=per.id_per and mov.id_usuario=us.id_usuario and mov.nro_document='".$des_document."' and mov.id_mov=".$idAutoDes." and tmov.cod_tipo_mov='51' and mov.status='1' order by bn.cod_bien asc";
	       	$rs=$this->ejecuta($sql);
		}
		...
		$html='
			...
			*{
			...
			<h4 align="center">Desincorporación de Bienes Nacionales</h4>
            </div>
			</div>
			...
			<tr>
				...
				<table border="1">
					...
					<td border="1" colspan="0" id="negrita" >CARGO: '.$tupla2['nom_car'].' </td>
					 ...
					 <!--COMIENZO DE LA TABLA PARA EL BIEN NACIONAL -->
	    ...
	    while($row=$this->TraerArreglo($rs)){
            ...
            if($row['nom_marca']=="NINGUNA MARCA"){
                   $marca=" ";
                        ...
                        }if($row['observacion_mov']==""){
                        	$condicion="MAL ESTADO";
                        }else{
                        	$condicion=$row['observacion_mov'];
                        }
                 	$cantidad=1;
                 	...
                 	<table border="1" align="center" style="border-collapse:collapse;margin-left:-5px;margin-top:-1px;">
				        <tr align="center">
					       ...
					       <td width="92" rowspan="2">'.$condicion.'</td>
				        </tr>
            	...
            	$pdf->WriteHTML($html);
            // }else{
            // 	echo "<script>alert('NO HAY DESINCORPORACION POR ESTE NRO ')</script>";
            // 	return false;
            // }
	$footer="<page_footer>
		<style>

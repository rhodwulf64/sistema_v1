...
<div class="ventanaModal container" id="ventanaModal"  style="overflow:auto;">
	<div class="contenido">
		...
		<form method="POST" action="../../control/c_reportes/c_asignacion/c_asig.php" id="envio_Modal" name="envio_Modal"><br>
			         &nbsp;&nbsp;&nbsp;&nbsp;
				<input type="button" name="btnSalir" id="btnSalir" value="Volver" onclick="salirListar(),location.href='?accion=inicio'">
			

			<?php include_once("../../modelo/m_reportes/modelo_consulta_rep/m_consulta_asig.php"); 
				$obj_recep = new clsConsultaAsig();
			...
			<tbody id="datosAjax">
				<?php while($rs = $obj_recep->converArray($tupla)){
					...
					$contenido.='<tr class="tbody" onclick=ejecuta("'.$contador[$c].'")>
								...
								<td><button type="button" value="'.$contador[$c].'" title="clic para abrir la busqueda" id="Editar" onclick="ejecuta(this.value)"><span id="iconosE" class="icon-file-text"></span></button></td>
							</tr>';
					$c++; 
                    }
				}
